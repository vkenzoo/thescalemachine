...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 1. Insights primeiro com FILTERING NA META: spend > 0 (corta inativos)
    // 2. Limite menor (200) — usuário ranqueia ads relevantes, não milhares
    // 3. Filtro de IDs no /ads + creative bem reduzido (sem image_url)
    ...
    const insightsRes = await graphGet<{ data: MetaInsight[] }>(
      token, `/${adAccount.account_id}/insights`, {
        ...
        filtering: insightsFiltering,
        limit: 200,
      }, appSecret
    ).catch((e) => {
      console.error("[/api/meta/ads] insights call failed:", e?.message);
    ...
    const filtering = JSON.stringify([
      { field: "id", operator: "IN", value: adIdsWithData.slice(0, 200) },
    ]);

    // Adsets/campaigns só com IDs únicos vindos dos insights; bem menor
    const [structRes, adsetsRes, campaignsRes] = await Promise.all([
      graphGet<{ data: MetaAd[] }>(token, `/${adAccount.account_id}/ads`, {
        fields: "id,name,status,effective_status,adset_id,campaign_id,creative{thumbnail_url,video_id}",
        filtering,
        limit: 200,
      }, appSecret),
